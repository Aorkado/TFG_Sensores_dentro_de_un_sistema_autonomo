/* i2c - Write and read example

   Simple I2C example that shows how to change to another register and then
...
static const char *TAG = "i2c-read-another-register";
...
#define I2C_RECEPTION_DATA_LENGTH  	2			/*!< Size, in bytes, of the read buffer */				
#define I2C_TRANSMISSION_DATA_LENGTH  	1			/*!< Size, in bytes, of the read buffer */

#define LM75A_SENSOR_ADDR           	0b1001100		/*!< Slave address of the LM75A sensor */
#define LM75A_TEMP_OS_ADDR          	0x03			/*!< Register address of the overtemperature register of the LM75A sensor */
#define LM75A_TEMP_HYST_ADDR        	0x02			/*!< Register address of the hysteresis register of the LM75A sensor */
...
void i2c_create_config(uint16_t device_address, i2c_master_dev_handle_t *dev_handle, i2c_master_bus_handle_t *bus_handle) {
    ...
    i2c_device_config_t dev_cfg = {			// Device configuration.
        ...
        .device_address = device_address,		// Device address.
...
void app_main(void){
    ...
    uint8_t * LM75A_register = malloc(sizeof(uint8_t));
    uint8_t * temperature_data = malloc(2*sizeof(uint8_t));

    *LM75A_register = (uint8_t)LM75A_TEMP_OS_ADDR;
    i2c_master_transmit_receive(dev_handle, LM75A_register, I2C_TRANSMISSION_DATA_LENGTH, temperature_data, I2C_RECEPTION_DATA_LENGTH, I2C_MASTER_TIMEOUT_MS/portTICK_PERIOD_MS);
    printf("Overtemperature is: %f \n", data_to_temperature(temperature_data)); 
    vTaskDelay(100 / portTICK_PERIOD_MS);
    
    *LM75A_register = (uint8_t)LM75A_TEMP_HYST_ADDR;
    i2c_master_transmit_receive(dev_handle, LM75A_register, I2C_TRANSMISSION_DATA_LENGTH, temperature_data, I2C_RECEPTION_DATA_LENGTH, I2C_MASTER_TIMEOUT_MS/portTICK_PERIOD_MS);
    printf("Hysteresis is: %f \n", data_to_temperature(temperature_data)); 
